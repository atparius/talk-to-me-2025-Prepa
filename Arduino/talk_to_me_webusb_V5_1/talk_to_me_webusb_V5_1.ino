...
void setup()
{
  ...
  Serial.begin(115200);
  delay(20);
  Serial.println("Pico WebUSB started");
...
void echo_all(char chr)
{
  ...
  // Only write to WebUSB if connected
  if (TinyUSBDevice.mounted())
  {
    tud_vendor_write(0, &chr, 1);
  }
}
...
void loop()
{
  ...
  {
    uint8_t buf[64];
    uint16_t count = tud_vendor_read(0, buf, 64);
    if (count > 0)
    {
      String command = "";
