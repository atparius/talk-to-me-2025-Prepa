...
export default class DialogMachine extends TalkMachine {
	...
	dialogFlow(eventType = "default", button = -1) {
		...
		switch (this.nextState) {
			...
			case "choose-blue":
				this.fancyLogger.logMessage("blue was a good choice, press any button to continue");
				this.nextState = "can-speak";
				break;

			case "choose-yellow":
				this.fancyLogger.logMessage("yellow was a bad choice, press blue button to continue");
			...
			case "count-press":
				...
				if (this.buttonPressCounter > 3) {
					...
					this.speechText("you pressed " + this.buttonPressCounter + " time", [0, 0.8, 1]);
				}
				break;
			...
			default:
				this.fancyLogger.logWarning(`Sorry but State: "${this.nextState}" has no case defined`);
		}
	}
	...
	performPreliminaryTests() {
		...
		if (this.speechIsSpeaking === true) {
			this.fancyLogger.logWarning("im speaking, please wait until i am finished");
			return false;
		}
		if (this.nextState == "") {
